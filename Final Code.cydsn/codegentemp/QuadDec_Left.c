...
void QuadDec_Left_Init(void) 
{
        ...
        (void) CyIntSetVector(QuadDec_Left_ISR_NUMBER, & QuadDec_Left_ISR);
        /* Set the priority. */
...
void QuadDec_Left_Stop(void) 
{
    ...
    QuadDec_Left_SR_AUX_CONTROL &= (uint8) (~QuadDec_Left_INTERRUPTS_ENABLE);
...
int32 QuadDec_Left_GetCounter(void) 
{
    ...
    uint16 tmpCnt;
        ...
        hwCount = (int16) ((int32) tmpCnt - (int32) QuadDec_Left_COUNTER_INIT_VALUE);
        count = QuadDec_Left_count32SoftPart + hwCount;

        CyIntEnable(QuadDec_Left_ISR_NUMBER);
    #else 
        /* (QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_8_BIT) || 
        *  (QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_16_BIT)
        */
        #if (QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_8_BIT)
            tmpCnt = QuadDec_Left_Cnt8_ReadCounter();
        #else /* (QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_16_BIT) */
            tmpCnt = QuadDec_Left_Cnt16_ReadCounter();
        #endif  /* QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_8_BIT */

        count = (int32) ((int32) tmpCnt -
                (int32) QuadDec_Left_COUNTER_INIT_VALUE);
...
*
* Global variables:
*  QuadDec_Left_count32SoftPart - modified to set hi 16 bit for current
...
void QuadDec_Left_SetCounter(int32 value) 
{
        ...
        if (value >= 0)
        {
            count = (uint16) value + QuadDec_Left_COUNTER_INIT_VALUE;
        }
        else
        {
            count = QuadDec_Left_COUNTER_INIT_VALUE - (uint16)(-value);
        }
        #if (QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_8_BIT)
        ...
        #endif  /* QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_8_BIT */
    #else /* (QuadDec_Left_COUNTER_SIZE == QuadDec_Left_COUNTER_SIZE_32_BIT) */
        CyIntDisable(QuadDec_Left_ISR_NUMBER);
