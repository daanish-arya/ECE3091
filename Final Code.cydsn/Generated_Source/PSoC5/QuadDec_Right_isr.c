...
#if !defined(QuadDec_Right_isr__REMOVED) /* Check for removal by optimization */
...
void QuadDec_Right_isr_Start(void)
{
    ...
    /* Set the priority. */
    QuadDec_Right_isr_SetPriority((uint8)QuadDec_Right_isr_INTC_PRIOR_NUMBER);
...
void QuadDec_Right_isr_Stop(void)
{
    ...
    QuadDec_Right_isr_SetVector(&IntDefaultHandler);
}
...
{
    #ifdef QuadDec_Right_isr_INTERRUPT_INTERRUPT_CALLBACK
        QuadDec_Right_isr_Interrupt_InterruptCallback();
    #endif /* QuadDec_Right_isr_INTERRUPT_INTERRUPT_CALLBACK */ 
...
*   before the component has been started use QuadDec_Right_isr_StartEx instead.
* 
*   When defining ISR functions, the CY_ISR and CY_ISR_PROTO macros should be 
*   used to provide consistent definition across compilers:
*
...
void QuadDec_Right_isr_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)QuadDec_Right_isr__INTC_NUMBER] = address;
}
...
cyisraddress QuadDec_Right_isr_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)QuadDec_Right_isr__INTC_NUMBER];
}
...
*
*   Note calling QuadDec_Right_isr_Start or QuadDec_Right_isr_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after QuadDec_Right_isr_Start or QuadDec_Right_isr_StartEx has been called. 
...
uint8 QuadDec_Right_isr_GetState(void)
{
    ...
    return ((*QuadDec_Right_isr_INTC_SET_EN & (uint32)QuadDec_Right_isr__INTC_MASK) != 0u) ? 1u:0u;
}
